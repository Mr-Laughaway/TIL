...
> 현재 빅데이터 분석기술 중에서 가장 주목 받는 기술은 스파크 이다.
>
> **스파크**는 인메모리(In-Memory) 기반의 처리로 하둡의 맵리듀스에 비해서 100배 빠른 속도를 제공하고, 머신러닝, 그래프처리 등 빅데이터 분석을 위한 통합 컴포넌트를 제공한다.
>
> 스파크는 `자바Java`, `스칼라Scala`, `파이썬Python`, `R 인터페이스`등 다양한 언어를 지원하여 개발자에게 작업의 편의성을 제공한다. 하지만 언어마다 처리하는 속도 다르다. 따라서 성능을 위해서는 `스칼라`로 개발을 진행하는 것이 좋다.또한 `스칼라`로 작성할 경우 `자바` 보다 간결한 코드로 같은 작업을 표현할 수 있다. 또한 `스칼라`는 `JVM`에서 동작하기 때문에 기존에 사용하던 `자바 라이브러리`를 모두 사용할 수 있다.
...
- Hadoop 기반의 데이터 인프라는 자바 언어를 통해 MapReduce 연산 그리고 알고리즘을 구현. 그런데 이 때 자바는 코드가 너무 길어 생산성 그리고 가독성이 매우 떨어진다.
