...
<c:forEach var="row" items="${headers}">
  <tr><td colspan=5 height=1 background=./images/dotline.gif></td></tr>
  <tr><td>${row.bid} </td><td>     
...
<%  }
  //for(int i = numPerBlock; i > 0; i--) {
    ...
    <a href=./list.do?page=<%=pn%>>
          <% } else { %>
        <a href="./search.do?page=<%=pn%>&searchKey=${searchKey}&searchWord=${searchWord}">    
          <% } %>
...
<%    } else { %>
    ...
    <a href=./list.do?page=<%=pn%>><%=pn%></a>&nbsp;
          <% } else { %>
        <a href=./search.do?page=<%=pn%>&searchKey=${searchKey}&searchWord=${searchWord}><%=pn%></a>&nbsp;  
          <% } %>     
<%    }
  }
  if(currentBlock > 1) { %>
